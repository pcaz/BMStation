...
class BackupMySQL extends \mysqli {
	...
	public function __construct($options = array()) {
		...
		if($options['tables'] && count($options['tables']<>0)){
			foreach($options['tables'] as $name=>$table){
...
// 		echo '<p style="padding:0; margin:1px 10px; font-family:sans-serif;">'. $message .'</p>';
	...
	protected function sauvegarder() {
		...
		if($result_tables && $result_tables->num_rows) {
			...
			for($i=0; $i<$result_tables->num_rows;$i++){
				$val=$result_tables->fetch_object();	
			foreach($this->tables as $table=>$values){
					if($val->{'Name'}==$table)
						$this->tables[$table]=$val;
				}}
			}
			else {
				for($i=0; $i<$result_tables->num_rows;$i++){
					$val=$result_tables->fetch_object();
					$this->tables[$val->{'Name'}]=$val;
			}}
				
//			while($obj_table = $result_tables->fetch_object()) {
			foreach($this->tables as $obj_table){
					...
					while($obj_insert = $result_insert->fetch_object()) {
						...
						foreach($obj_insert as $val) {
							...
							if(is_null($val)) {
								...
								$sql .= '\''. $this->insert_clean(utf8_encode($val)) . '\'';
							}
							$virgule = true;
						} // for
